...
/*
 * zvram.c - GPU VRAM-backed storage backend for zswap
 *
...
/*
 * Read staging buffers.  VRAM is iomem and cannot be handed to the zswap
 * decompressor directly, so read_begin() bounces the compressed object into a
 * host-RAM buffer and points the scatterlist at it.  zswap holds that buffer
 * across the (sleeping) decompress, so a per-CPU/local_lock buffer is unsafe;
 * a mempool guarantees a buffer without per-read allocation failure.  PAGE_SIZE
 * is the largest object zswap stores, plus 16 for drm_memcpy_from_wc padding.
 */
#define ZVRAM_STAGE_SIZE	(PAGE_SIZE + 16)
static mempool_t *zvram_stage_pool;
...
/* Non-NULL token returned by zvram_be_create() to identify the pool. */
...
/*********************************
* zswap storage backend
**********************************/

static void *zvram_be_create(const char *name)
{
	unsigned int i;
...
static void zvram_be_destroy(void *pool)
{
	/* GPU lifecycle is managed by module init/exit and DRM callbacks. */
}

static unsigned long zvram_be_malloc(void *pool, size_t size, gfp_t gfp, int nid)
{
	struct zvram_gpu *gpu;
	unsigned int order;
	long offset;
	...
	return zvram_make_handle(gpu->gpu_idx, offset, size);
}

static void zvram_be_free(void *pool, unsigned long handle)
{
...
static struct zvram_gpu *zvram_handle_gpu_ready(unsigned long handle)
{
	int gpu_idx = zvram_handle_gpu(handle);
...
static void zvram_be_write(void *pool, unsigned long handle, void *buf,
			   size_t len)
{
	struct zvram_gpu *gpu = zvram_handle_gpu_ready(handle);

	if (gpu)
		zvram_write_to_vram(gpu, zvram_handle_offset(handle), buf, len);
}

/*
 * VRAM is iomem and cannot be handed to the (sleeping) zswap decompressor
 * directly, so bounce the compressed object into a host-RAM staging buffer
 * with drm_memcpy_from_wc() and point the scatterlist at it.  zswap holds the
 * sg across the decompress and then calls read_end(), which frees the buffer.
 */
static void zvram_be_read_begin(void *pool, unsigned long handle,
				struct scatterlist *sg, size_t len)
{
	...
	if (gpu)
		zvram_read_from_vram(gpu, zvram_handle_offset(handle), buf, len);
	else
		memset(buf, 0, len);	/* stale handle: fail decompress, not crash */
	sg_init_one(sg, buf, len);
}

static void zvram_be_read_end(void *pool, unsigned long handle,
...
static u64 zvram_be_total_pages(void *pool)
{
	...
	for (i = 0; i < zvram_nr_gpus; i++) {
		...
		if (gpu)
			bytes += atomic_long_read(&gpu->used_bytes);
	}
...
static void zvram_drm_unregister(struct drm_client_dev *client)
{
	...
	/*
	 * Phase 2 (TODO): drain entries stored on this GPU back to disk swap.
	 * The zpool framework's zpool_drain_by_driver() did this; with the
	 * direct zswap backend a small zswap_writeback_backend() extension is
	 * the clean equivalent.  Until then, GPU hot-unbind while entries are
	 * resident will make those entries fail to load (read_begin() below
	 * fails the decompress rather than reading freed VRAM) -- a loud
	 * failure, not silent corruption.  This does not occur in normal
	 * operation where the GPU stays bound for the module's lifetime.
	 */
	WARN_ON_ONCE(atomic_long_read(&gpu->used_bytes) != 0);

	/*
	 * Phase 3: Tear down the pool.  After this point a handle that still
	 * references this GPU fails its load loudly rather than corrupting.
	 *
...
{
	return drm_dev_get_by_parent(&pdev->dev);
}
...
static int __init zvram_init(void)
{
	...
	pr_info("initializing zvram zswap backend\n");

	/* Initialize per-NUMA-node preferred GPU map */
	for (i = 0; i < MAX_NUMNODES; i++)
		zvram_pref_gpu[i] = -1;

	/* Read staging buffers for the VRAM->RAM bounce on the load path. */
	zvram_stage_pool = mempool_create_kmalloc_pool(num_possible_cpus() * 2,
						       ZVRAM_STAGE_SIZE);
	if (!zvram_stage_pool)
		return -ENOMEM;
	...
	/* Register as the "zvram" zswap storage backend. */
	ret = zswap_register_backend(&zvram_backend);
	if (ret) {
		pr_err("failed to register zswap backend: %d\n", ret);
		goto fail_backend;
	}
...
fail_backend:
	zvram_debugfs_exit();
	mempool_destroy(zvram_stage_pool);
	return ret;
}
...
static void __exit zvram_exit(void)
{
	...
	zswap_unregister_backend(&zvram_backend);
...
MODULE_DESCRIPTION("GPU VRAM-backed storage backend for zswap");
