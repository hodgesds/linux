				      ...
				      spinlock_t *lock, void *arg)
{
	 ...
	 *
	 * It's safe to drop the lock here because we return either
	 * LRU_REMOVED_RETRY or LRU_RETRY; the caller will reacquire it.
	 */
	swpoffset = swp_offset(entry->swpentry);
	tree = zswap_trees[swp_type(entry->swpentry)];
	list_lru_isolate(l, item);
	...
	if (writeback_result) {
		/*
		 * Writeback failed: putback re-adds the entry to the LRU,
		 * so we must tell list_lru_walk that the item is not gone
		 * by returning LRU_RETRY rather than LRU_REMOVED_RETRY --
		 * otherwise nlru->nr_items drifts on every failed drain.
		 */
		zswap_reject_reclaim_fail++;
		zswap_lru_putback(&entry->pool->list_lru, entry);
		ret = LRU_RETRY;
			       ...
			       void *filter_data)
{
	...
	struct zswap_pool **pools, *pool;
	size_t nr_pools = 0;
	int np = 0, i, nid;

	/*
	 * Two-pass: first count matching pools under RCU so we can
	 * allocate exactly the right array size, then re-walk and take
	 * a reference on each.  This avoids the fixed cap that silently
	 * dropped pools on the floor on systems with many zswap
	 * instances.  Allocation happens outside RCU.
	 */
	rcu_read_lock();
	list_for_each_entry_rcu(pool, &zswap_pools, list) {
		if (!strcmp(zpool_get_type(pool->zpools[0]), type))
	...
	pools = kmalloc_array(nr_pools, sizeof(*pools), GFP_KERNEL);
	if (!pools)
		return -ENOMEM;
