...
static void __sched_fork(u64 clone_flags, struct task_struct *p)
{
	...
	RB_CLEAR_NODE(&p->microd.run_node);
	p->microd.vruntime = 0;
	p->microd.min_vruntime = 0;
	p->microd.microd_prio = 0;
	p->microd.on_rq = 0;
	p->microd.tgid_ctx = NULL;
	p->microd.prev_llc = -1;
	p->microd.last_sleep_duration = 0;
	p->microd.total_sleep_ns = 0;
	p->microd.total_run_ns = 0;
	p->microd.interactive = 0;
#endif
...
int sched_fork(u64 clone_flags, struct task_struct *p)
{
	...
	if (rt_prio(p->prio)) {
		...
		p->sched_class = &microd_sched_class;
	}
#else
	} else {
		p->sched_class = &fair_sched_class;
...
#ifdef CONFIG_SCHED_CLASS_MICROD
	/*
	 * Microd fast path: when only microd+CFS tasks exist and prev is
	 * at or below microd class, pick directly from the microd rb-tree.
	 * This avoids prev_balance() and iterating stop/dl/rt pick_task
	 * callbacks, saving ~3 indirect calls per schedule.
	 */
	if (!sched_class_above(prev->sched_class, &microd_sched_class) &&
	    rq->nr_running == rq->microd.nr_running + rq->cfs.h_nr_queued) {
		if (rq->microd.nr_running) {
			struct rb_node *left;

			left = rb_first_cached(&rq->microd.tasks_timeline);
			if (likely(left)) {
				struct sched_microd_entity *me;

				me = rb_entry(left, struct sched_microd_entity,
					      run_node);
...
#ifdef CONFIG_SCHED_CLASS_MICROD
	/*
	 * microd_policy check must come before rt_prio because microd
	 * tasks use RT-range priority values but belong to microd class.
	 */
	if (microd_policy(policy))
		return &microd_sched_class;
#endif
...
#ifdef CONFIG_SCHED_CLASS_MICROD
	/* microd takes over all fair tasks when enabled */
	return &microd_sched_class;
#else
...
#ifdef CONFIG_SCHED_CLASS_MICROD
	BUG_ON(!sched_class_above(&rt_sched_class, &microd_sched_class));
	BUG_ON(!sched_class_above(&microd_sched_class, &fair_sched_class));
#endif
#ifdef CONFIG_SCHED_CLASS_EXT
