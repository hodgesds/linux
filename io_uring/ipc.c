			   ...
			   const struct io_uring_ipc_channel_create __user *arg)
{
	...
	mutex_init(&channel->producer_lock);
			   ...
			   const struct io_uring_ipc_channel_attach __user *arg)
{
	...
	ret = xa_insert(&channel->subscribers, sub->subscriber_id, sub, GFP_ATOMIC);
...
int io_ipc_recv(struct io_kiocb *req, unsigned int issue_flags)
{
	...
	/*
	 * Find subscriber under RCU and copy fields we need into locals.
	 * After rcu_read_unlock(), the subscriber can be freed by a
	 * concurrent detach from the register path (which only needs
	 * uring_lock, not held on the io-wq retry path).
	 */
	rcu_read_lock();
	list_for_each_entry_rcu(sub, &req->ctx->ipc_subscriber_list, list) {
...
found:
	...
	/*
	 * Re-find subscriber under RCU to safely write back local_head.
	 * If the subscriber was detached while we were copying, skip the
	 * update -- the subscriber is going away.
	 */
	rcu_read_lock();
	list_for_each_entry_rcu(sub, &req->ctx->ipc_subscriber_list, list) {
		if (sub->subscriber_id == sub_id)
			goto update_head;
	}
	rcu_read_unlock();
	ret = copy_len;
...
update_head:
	...
	if (channel->flags & IOIPC_F_BROADCAST) {
		/*
		 * In broadcast mode, advance the global consumer head to the
		 * minimum local_head across all subscribers.  This tells the
		 * sender which slots have been consumed by every receiver and
		...
		/* Non-broadcast: single consumer, update directly */
